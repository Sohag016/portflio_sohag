...
export const researchPapers: ResearchPaper[] = [
  ...
  },
  {
    ...
    venue: "Journal of King Saud University Computer and Information Sciences, Springer Nature — Manuscript Under Review",
    abstract:
      "Abstract—The rapid dissemination of multimodal news content through online platforms has significantly increased the complexity of verifying misinformation. Most existing multimodal fake-news detection methods treat this problem as a straightforward classification problem, focusing primarily on label prediction. As a result, they provide limited insight into whether text, images, and image-embedded evidence corroborate one another and do not explicitly model cross-modal consistency, a key indicator of misinformation in real social media environments. To overcome these limitations, this paper proposes an Explainable Multimodal Consistency Reasoning (EMCR) framework for multimodal news verification. EMCR extends conventional multimodal misinformation detection by introducing a consistency-estimation stage that complements supervised prediction with explicit cross-modal reasoning. The framework jointly analyzes three complementary evidence streams: (i) text-image semantic consistency learned through a pretrained vision-language model, (ii) OCR-guided textual consistency between news titles and embedded text, and (iii) metadata-based structural evidence derived from image-level statistical characteristics. These modality-specific signals are integrated through a weighted fusion mechanism that yields a unified score quantifying agreement across information sources, while a separate learned classifier combines the branch-level predictions for category verification. EMCR further includes an explainable reasoning module revealing each modality’s contribution. This allows for a transparent interpretation of verification results and identifies whether inconsistencies arise from semantic mismatch, OCR-based contradiction, or insufficient structural support. Experimental evaluations on the Fakeddit benchmark dataset show that the proposed EMCR fusion classifier, which combines the supervised RoBERTa–ResNet50 baseline, the CLIP-based semantic branch, and the XGBoost metadata branch, achieves 77.02% Accuracy and 76.82% Macro F1-score, outperforming each individual branch evaluated in isolation (RoBERTa–ResNet50: 71.12% / 70.94%; CLIP ViT-B/32: 70.46% / 69.53%; XGBoost metadata: 57.11% / 56.89%). Beyond improved predictive performance, EMCR provides transparent modality-level reasoning and a confidence-aware verification score, with retained-sample accuracy exceeding 96% at consistency thresholds τ ≥ 0.60 and reaching 100% for τ ≥ 0.75. Component-wise evaluations show the differing strengths of semantic, OCR-based, and structural evidence sources.",
    pdfUrl:
      "https://drive.google.com/file/d/1d5OBleYs5zQg_abf8_q-zhgTVBkGvD8B/view?usp=sharing",
    doiUrl: "#",
    citationsCount: 0,
  },
  {
